...
if __name__ == "__main__":
    ...
    print("Running checkpoint manager test...")

    with tempfile.TemporaryDirectory() as tmp:
        model     = nn.Linear(10, 5)
        optimizer = torch.optim.Adam(model.parameters(), lr=3e-4)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer)
        ckpt      = CheckpointManager(tmp, metric_name="val_f1")
        es        = EarlyStopping(patience=3, mode="max")

        # Simulate 6 epochs
        scores = [0.30, 0.35, 0.40, 0.39, 0.38, 0.37]
        best   = 0.0
        for epoch, score in enumerate(scores):
            is_best = score > best
            if is_best:
                best = score
            ckpt.save(epoch, model, optimizer, scheduler, score, is_best=is_best)
            if es.step(score):
        ...
        start, metric = ckpt.load_latest(model2, optimizer2, scheduler2)
        print(f"  Resumed: start_epoch={start}, last_val_f1={metric:.4f}")

        # Load best
        model3 = nn.Linear(10, 5)
        best_metric = ckpt.load_best(model3)
        print(f"  Best val_f1 from checkpoint: {best_metric:.4f}")
        print(f"  Best val_f1 from log        : {ckpt.best_metric():.4f}")
        print(f"  Training log: {ckpt.get_log()}")
